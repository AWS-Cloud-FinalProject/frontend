...
const UserEditModal = ({ setModal }) => {
  ...
  const editPwFn = async () => {
    ...
    else if (repeatNewPw === '') return alert('새 비밀번호 확인을 입력해 주세요.')
    else if (newPw !== repeatNewPw) return alert('비밀번호 확인을 제대로 입력해 주세요.')
    else {
      const result = await editPw(currentPw, newPw)
      if (typeof result === 'object' && result?.data?.message === 'User password updated successfully') {
        alert('비밀번호가 변경되었습니다.')
        ...
        {step === 'changePw' &&
          <>
            <div className="column input-form change-pw">
              <span>비밀번호 변경</span>
              <input type="password" placeholder="기존 비밀번호" value={currentPw}
                     onChange={e => setCurrentPw(e.target.value)} />
              <input type="password" placeholder="새 비밀번호" value={newPw} onChange={e => setNewPw(e.target.value)} />
              <input type="password" placeholder="새 비밀번호 확인" value={repeatNewPw}
                     onChange={e => setRepeatNewPw(e.target.value)} />
              {newPw && repeatNewPw && newPw !== repeatNewPw ? <p>비밀번호를 제대로 입력해 주세요.</p> : ''}
              <button className="change-pw-btn" onClick={editPwFn}>비밀번호 변경</button>
            </div>
          </>}
        {step === 'withdraw' &&
          <>
            <div className="column input-form withdraw">
              <span>회원 탈퇴</span>
              <input type="password" value={currentPw} placeholder="비밀번호 입력"
                     onChange={e => setCurrentPw(e.target.value)} />
              <div className="withdraw-check row">
                <input type="checkbox" id="withdraw-check" checked={withdrawCheck}
                       onChange={e => setWithdrawCheck(e.target.checked)} />
                <label for="withdraw-check">정말로 회원 탈퇴를 하시겠습니까?</label>
              </div>
              <button className="withdraw-btn" onClick={withdrawFn}>회원 탈퇴</button>
            </div>
          </>}
      </div>
    </div>
