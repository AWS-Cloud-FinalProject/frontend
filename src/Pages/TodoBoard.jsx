...
const TodoBoard = () => {
  ...
  const getTodoList = async () => {
    ...
    try {
      ...
      } else setTasks({ todo: [], inProgress: [], review: [], done: [] })
    } catch (error) {
  ...
  const createTodoFn = async () => {
    if (createTask.title === '') return alert('할 일 제목을 입력해 주세요.')
    else if (createTask.contents === '')
      return alert('할 일 내용을 입력해 주세요.')
    else {
      ...
      if (
        typeof result === 'object' &&
        result?.data?.message === 'Todo Create Successfully'
      ) {
        setCreate(false)
        setCreateTask({
          title: '',
  ...
  const onDragEnd = async result => {
    ...
    // tasks 객체가 존재하지 않거나, 해당 컬럼이 없으면 기본값 설정
    if (!tasks || typeof tasks !== 'object') {
      return
    }

    if (!(sourceColumn in tasks) || !(destinationColumn in tasks)) {
      return
    }

    // 같은 컬럼 내에서 순서만 변경하는 경우
    if (sourceColumn === destinationColumn) {
      const updatedTasks = Array.isArray(tasks[sourceColumn])
        ? [...tasks[sourceColumn]]
        : []
      if (source.index >= updatedTasks.length) return // 유효한 인덱스인지 확인
      ...
      await editTodoFn('orderEdit', {
        ...movedItem,
        todo_order: destination.index,
        status: destinationColumn,
      })
    }
    // 다른 컬럼으로 이동하는 경우
    else {
      const sourceTasks = Array.isArray(tasks[sourceColumn])
        ? [...tasks[sourceColumn]]
        : []
      const destinationTasks = Array.isArray(tasks[destinationColumn])
              ...
              Object.entries(tasks).map(([columnId, columnTasks]) => (
                <Droppable key={columnId} droppableId={columnId}>
                  {provided => {
                    ...
                    const safeColumnTasks = Array.isArray(columnTasks)
                      ? columnTasks
                      : []
                    ...
                    return (
                      ...
                      >
                        <div className='title'>
                          <span>
                            {columnId.toUpperCase() === 'INPROGRESS'
                              ? 'IN-PROGRESS'
                        ...
                        {safeColumnTasks.map(
                          ({ title, contents, todo_num, todo_order }, idx) => (
                            <Draggable
                              key={`${columnId}-${todo_num}`}
                              draggableId={`${columnId}-${todo_num}`}
                              index={idx}
                            >
                              {provided =>
                                editTask.todo_num === todo_num && edit ? (
                                  <div className='list edit-task-form'>
                                    <span className={`list-status ${columnId}`}>
                                      {columnId}
                                    </span>
                                    <input
                                      type='text'
                                  ...
                                  >
                                    <div className='row list-top'>
                                      <span
                                        className={`list-status ${columnId}`}
                                      >
                                        {columnId}
                                      </span>
                                      <div className='row'>
